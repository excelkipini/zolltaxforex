...
interface CashierPendingTransactionsByTypeProps {
  user: { name: string; role: string }
  transactionType: "reception" | "exchange" | "transfer" | "receipt"
}

export function CashierPendingTransactionsByType({ user, transactionType }: CashierPendingTransactionsByTypeProps) {
  ...
  React.useEffect(() => {
    const loadTransactions = async () => {
      try {
        ...
        if (res.ok && data?.ok && Array.isArray(data.data)) {
          ...
          // Filtrer les transactions validées créées par ce caissier du type spécifique
    ...
    const handleTransactionCreated = () => loadTransactions()
  ...
  const getSectionTitle = () => {
    switch (transactionType) {
      case "reception":
        return "Mes Transactions Réception/Envoi à Clôturer"
      case "exchange":
        return "Mes Transactions Bureau de Change à Clôturer"
      case "transfer":
        return "Mes Transactions Transfert d'Argent à Clôturer"
      case "receipt":
        return "Mes Reçus d'Opération à Clôturer"
      default:
        return "Mes Transactions à Clôturer"
    }
  }
      ...
      <CardContent>
        {transactions.length === 0 ? (
          <div className="text-center py-6 text-gray-500">
            ...
            <p>Aucune transaction {getTypeLabel(transactionType).toLowerCase()} à clôturer</p>
            <p className="text-sm">Toutes vos transactions validées de ce type ont été clôturées</p>
          </div>
        ) : (
        ...
        <div className="mt-4 p-3 bg-green-50 rounded-lg">
          <p className="text-sm text-green-800">
            <strong>Note :</strong> Ces transactions {getTypeLabel(transactionType).toLowerCase()} ont été validées par un auditeur et sont prêtes à être clôturées. 
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Clôturer toutes les transactions {getTypeLabel(transactionType).toLowerCase()} validées
            </DialogTitle>
            <DialogDescription>
              Êtes-vous sûr de vouloir clôturer toutes les transactions {getTypeLabel(transactionType).toLowerCase()} validées ? 
