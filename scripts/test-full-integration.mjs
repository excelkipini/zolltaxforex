...
/**
 * Script de test pour vérifier l'intégration complète dans l'application
 * 
 * Usage: node scripts/test-full-integration.mjs
 */
...
async function testFullIntegration() {
  try {
    console.log('🧪 Test de l\'intégration complète dans l\'application...')
    ...
    // Test 2: Créer une transaction comme le ferait l'application
    console.log('\n📝 Test 2: Création d\'une transaction comme dans l\'application...')
    
    const transactionId = `FULL_INTEGRATION_${Date.now()}`
    const transactionData = {
      id: transactionId,
      type: 'transfer',
      status: 'pending',
      description: 'Test intégration complète - Transfert vers Dakar',
      amount: 400000,
      currency: 'XAF',
      created_by: 'Stevie Kibila', // Utilisateur caissier
      agency: 'Agence Centrale',
      details: JSON.stringify({
        recipient: 'Fatou Diop',
    ...
    // Test 3: Simuler l'envoi d'email comme le ferait l'application
    console.log('\n📧 Test 3: Simulation de l\'envoi d\'email...')
...
<body>
    ...
    console.log('\n💡 Si les emails ne sont pas reçus depuis l\'application:')
    console.log('   1. Vérifiez que le serveur Next.js est démarré')
    console.log('   2. Vérifiez les logs du serveur pour les erreurs')
    console.log('   3. Vérifiez que les variables d\'environnement sont chargées dans l\'application')
    console.log('   4. Vérifiez que la fonction createTransaction est bien appelée')
